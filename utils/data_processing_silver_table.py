...
class FinancialsSilverProcessor(BaseSilverProcessor):
    ...
    def get_validation_config(self):
        ...
        return {
            ...
            "Num_Fin_Pdts": InvalidDataStrategy.FLAG_ONLY,        # Track engineered feature quality
            "Loans_per_Credit_Item": InvalidDataStrategy.FLAG_ONLY,  # Track engineered ratio quality
            "Debt_to_Salary": InvalidDataStrategy.FLAG_ONLY,      # Track engineeered debt ratio quality
            "EMI_to_Salary": InvalidDataStrategy.FLAG_ONLY,       # Track engineered affordability ratio
            "Repayment_Ability": InvalidDataStrategy.FLAG_ONLY,   # Track net repayment buffer
            "Loan_Extent": InvalidDataStrategy.FLAG_ONLY,         # Track payment delay magnitude
        }
    ...
    def apply_business_logic(self, df):
        ...
        # Engineer total count of financial products held by the customer
        financial_count_columns = ["Num_Bank_Accounts", "Num_Credit_Card", "Num_of_Loan"]
        if all(col_name in df.columns for col_name in financial_count_columns):
            total_financial_products = (
                F.coalesce(col("Num_Bank_Accounts"), F.lit(0)) +
                F.coalesce(col("Num_Credit_Card"), F.lit(0)) +
                F.coalesce(col("Num_of_Loan"), F.lit(0))
            )
            df = df.withColumn(
                "Num_Fin_Pdts",
                F.when(
                    col("Num_Bank_Accounts").isNull() &
                    col("Num_Credit_Card").isNull() &
                    col("Num_of_Loan").isNull(),
                    F.lit(None)
                ).otherwise(total_financial_products).cast(IntegerType())
            )

            # Validate engineered ratio remains non-negative for ML consumption
            fin_pdts_strategy = config.get("Num_Fin_Pdts", InvalidDataStrategy.FLAG_ONLY)
            fin_pdts_default = 0 if fin_pdts_strategy == InvalidDataStrategy.DEFAULT_VALUE else None
            df = self.validate_and_handle_non_negative_amount(df, "Num_Fin_Pdts", fin_pdts_strategy, fin_pdts_default)

            # Engineer loans per credit item ratio (guards against division by zero/null)
            credit_items_total = (
                F.coalesce(col("Num_Bank_Accounts"), F.lit(0)) +
                F.coalesce(col("Num_Credit_Card"), F.lit(0))
            )
            df = df.withColumn(
                ...
                F.when(
                    ...
                    (credit_items_total > F.lit(0)),
                    col("Num_of_Loan") / credit_items_total
                ).otherwise(F.lit(None)).cast(FloatType())
            )

            loans_per_credit_strategy = config.get("Loans_per_Credit_Item", InvalidDataStrategy.FLAG_ONLY)
            loans_per_credit_default = 0.0 if loans_per_credit_strategy == InvalidDataStrategy.DEFAULT_VALUE else None
            df = self.validate_and_handle_non_negative_amount(
                df,
                "Loans_per_Credit_Item",
                loans_per_credit_strategy,
                loans_per_credit_default
            )

        # Engineer debt to salary ratio when both inputs are present and salary > 0
        if {"Outstanding_Debt", "Monthly_Inhand_Salary"}.issubset(df.columns):
            df = df.withColumn(
                "Debt_to_Salary",
                F.when(
                    col("Outstanding_Debt").isNotNull() &
                    col("Monthly_Inhand_Salary").isNotNull() &
                    (col("Monthly_Inhand_Salary") > F.lit(0)),
                    col("Outstanding_Debt") / col("Monthly_Inhand_Salary")
                ).otherwise(F.lit(None)).cast(FloatType())
            )

            debt_to_salary_strategy = config.get("Debt_to_Salary", InvalidDataStrategy.FLAG_ONLY)
            debt_to_salary_default = 0.0 if debt_to_salary_strategy == InvalidDataStrategy.DEFAULT_VALUE else None
            df = self.validate_and_handle_non_negative_amount(
                df,
                "Debt_to_Salary",
                debt_to_salary_strategy,
                debt_to_salary_default
            )

        # Engineer EMI to salary ratio when both inputs are present and salary > 0
        if {"Total_EMI_per_month", "Monthly_Inhand_Salary"}.issubset(df.columns):
            ...
            emi_to_salary_strategy = config.get("EMI_to_Salary", InvalidDataStrategy.FLAG_ONLY)
            emi_to_salary_default = 0.0 if emi_to_salary_strategy == InvalidDataStrategy.DEFAULT_VALUE else None
            df = self.validate_and_handle_non_negative_amount(
                df,
                "EMI_to_Salary",
                emi_to_salary_strategy,
                emi_to_salary_default
            )

        # Engineer repayment ability (monthly surplus after EMI payments)
        if {"Monthly_Inhand_Salary", "Total_EMI_per_month"}.issubset(df.columns):
            df = df.withColumn(
                "Repayment_Ability",
                F.when(
                    col("Monthly_Inhand_Salary").isNotNull() &
            ...
            df = df.withColumn(
                "valid_Repayment_Ability",
                col("Repayment_Ability").isNotNull()
            )

            repay_strategy = config.get("Repayment_Ability", InvalidDataStrategy.FLAG_ONLY)
            repay_default = 0.0 if repay_strategy == InvalidDataStrategy.DEFAULT_VALUE else None
            df = self._handle_invalid_data(
                df,
                "Repayment_Ability",
                "valid_Repayment_Ability",
                repay_strategy,
        ...
        # Engineer loan extent (combined delay impact across active loans)
        if {"Delay_from_due_date", "Num_of_Loan"}.issubset(df.columns):
            df = df.withColumn(
                "Loan_Extent",
                F.when(
                    col("Delay_from_due_date").isNotNull() &
                    col("Num_of_Loan").isNotNull(),
            ...
            df = df.withColumn(
                "valid_Loan_Extent",
                col("Loan_Extent").isNotNull()
            )

            loan_extent_strategy = config.get("Loan_Extent", InvalidDataStrategy.FLAG_ONLY)
            loan_extent_default = 0 if loan_extent_strategy == InvalidDataStrategy.DEFAULT_VALUE else None
            df = self._handle_invalid_data(
                df,
                "Loan_Extent",
                "valid_Loan_Extent",
                loan_extent_strategy,
                loan_extent_default
            )
